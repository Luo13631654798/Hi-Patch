...
def tensorize_normalize_extract_feature_patch(P, y, mf, stdf, ms, ss, args):
    ...
    for b in range(P_tensor.shape[0]):
        ...
        if args.dataset == 'physionet' or args.dataset == 'P12' or args.dataset == 'mimic3':
    ...
    observed_data =  pad_sequence(combined_vals, batch_first=True).permute(0, 2, 1, 3)  # (n_batch, T_o, D)
    observed_tp =  pad_sequence(combined_tt, batch_first=True).unsqueeze(-1).repeat(1, 1, 1, F).permute(0, 2, 1,3)  # (T_o, )
    observed_mask = pad_sequence(combined_mask, batch_first=True).permute(0, 2, 1, 3)  # (n_batch, T_o, D)

    return observed_data, observed_mask, torch.FloatTensor(P_static_tensor), observed_tp, y_tensor

def tensorize_normalize_exact_feature_mimic3_patch(P, y, mf, stdf, args):
    T, F = 292, 16
    ...
    observed_data = pad_sequence(combined_vals, batch_first=True).permute(0, 2, 1, 3)  # (n_batch, T_o, D)
    observed_tp = pad_sequence(combined_tt, batch_first=True).unsqueeze(-1).repeat(1, 1, 1, F).permute(0, 2, 1,
                                                                                                       3)  # (T_o, )
    observed_mask = pad_sequence(combined_mask, batch_first=True).permute(0, 2, 1, 3)  # (n_batch, T_o, D)

    return observed_data, observed_mask, None, observed_tp, y_tensor
    # return torch.FloatTensor(P_tensor), None, , torch.Tensor(P_time_tensor), y_tensor
...
def evaluate_model_patch(model, P_tensor, P_mask_tensor, P_static_tensor, P_time_tensor,
    ...
    for i in range(n_batches):
        ...
        middleoutput = model.classification(P, P_time, P_mask, Pstatic)
    ...
    if rem > 0:
        ...
        whatever = model.classification(P, P_time, P_mask, Pstatic)
